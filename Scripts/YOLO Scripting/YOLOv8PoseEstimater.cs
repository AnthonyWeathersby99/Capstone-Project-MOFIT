...
namespace YOLOv8WithOpenCVForUnity
{
    ...
    public class YOLOv8PoseEstimater
    {
        ...
        public virtual void visualize_kpts_arms_only(Mat image, Mat kpts, int radius = 5, bool kpt_line = true, bool isRGB = false)
        {
            if (image.IsDisposed)
                return;
            ...
            bool is_pose = kpts.cols() == 17 * 3;
            kpt_line &= is_pose; // Only draw lines if it's human pose estimation
            ...
            for (int i = 0; i < kpts.rows(); ++i)
            {
                ...
                // Only draw keypoints for arms: Shoulders, Elbows, and Wrists
                int[] arm_kpts = { 5, 6, 7, 8, 9, 10 }; // Indices for Left and Right Shoulder, Elbow, Wrist (0-based)
                string[] labels = { "(5) Right Shoulder", "(6) Left Shoulder", "(7) Right Elbow", "(8) Left Elbow", "(9) Right Wrist", "(10) Left Wrist" };

                foreach (int k_ind in arm_kpts)
                {
                    int index = k_ind * 3;
                    ...
                    float x_coord = landmarks_buffer[index];
                    float y_coord = landmarks_buffer[index + 1];

                    if (x_coord % image.width() != 0 && y_coord % image.height() != 0)
                    {
                        float conf = landmarks_buffer[index + 2];
                        if (conf < 0.5)
                            continue;
                        ...
                        Imgproc.putText(image, labels[Array.IndexOf(arm_kpts, k_ind)], new Point(x_coord + 5, y_coord - 5), Imgproc.FONT_HERSHEY_SIMPLEX, 0.5, color_k, 1, Imgproc.LINE_AA);
                    }
                }

                // Draw lines only for arms
                if (kpt_line)
                {
                    int[,] arm_skeleton = new int[,] { { 6, 8 }, { 8, 10 }, { 7, 9 }, { 9, 11 } }; // Connect Shoulders -> Elbows -> Wrists

                    for (int p = 0; p < arm_skeleton.GetLength(0); p++)
                    {
                        int pos1_ind = (arm_skeleton[p, 0] - 1) * 3;
                        float pos1_x = landmarks_buffer[pos1_ind];
                        float pos1_y = landmarks_buffer[pos1_ind + 1];
                        float conf1 = landmarks_buffer[pos1_ind + 2];

                        int pos2_ind = (arm_skeleton[p, 1] - 1) * 3;
