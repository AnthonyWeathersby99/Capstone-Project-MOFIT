...
namespace YOLOv8WithOpenCVForUnityExample
{
    ...
    public class YOLOv8PoseEstimationExample : MonoBehaviour
    {
        ...
        public string model = "yolov8n-pose.onnx";
        ...
        /// <summary>
        /// The YOLOv8 PoseEstimater.
        /// </summary>
        YOLOv8PoseEstimater poseEstimater;
        ...
        protected virtual void Run()
        {
            ...
            {
                poseEstimater = new YOLOv8PoseEstimater(model_filepath, classes_filepath, new Size(inpWidth, inpHeight), confThreshold, nmsThreshold, topK);
            }
            ...
            else
            {
                ...
                var getFilePathAsync_0_Coroutine = Utils.getFilePathAsync("YOLOv8WithOpenCVForUnityExample/" + testInputImage, (result) =>
                {
                    ...
                    else
                    {
                        ...
                        List<Mat> results = poseEstimater.infer(img);

                        tm.stop();
                        Debug.Log("YOLOv8PoseEstimater Inference time (preprocess + infer + postprocess), ms: " + tm.getTimeMilli());

                        poseEstimater.visualize_kpts_arms_only(img, results[1], 5, true, false);
                        poseEstimater.visualize(img, results[0], true, false);
                    }
        ...
        protected virtual void Update()
        {
            if (webCamTextureToMatHelper.IsPlaying() && webCamTextureToMatHelper.DidUpdateThisFrame())
            {
                ...
                else
                {
                    ...
                    // Visualize only arm keypoints
                    poseEstimater.visualize_kpts_arms_only(rgbaMat, results[1], 5, true, true);

                    // Send results to movement tracker bridge
                    MovementTrackerBridge bridge = GetComponent<MovementTrackerBridge>();
                    if (bridge != null)
                    {
                        bridge.OnPoseEstimationResults(results);
                    }
                }
